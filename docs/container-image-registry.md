...
그렇게 하기 위해서, 여러분이 직접 build한 container image를 Docker Hub에 push하고 pull 하는 대신에, 직접 구축한 kubernetes cluster에 Docker Hub와 같은 역할을 하는 Container Image Registry를 구축햐야 합니다.  
...
아래의 명령어를 입력하여 Persistent Volume을 정의해둔 yaml 파일을 확인하겠습니다.  
...
> [!NOTE]
> Persist volume claim은 사용자가 특정 조건을 만족하는 스토리지(PV)를 사용하겠다고 선언하는 kubernetes 리소스입니다. 일반적으로 특정 기능을 하는 Pod가 스토리지를 요구할 때, 사전에 정의된 Persistent volume을 사용할 수 있도록 합니다. Persistent volume claim을 통해서 사전 정의된 PV를 사용할 수 있게 되는 것입니다.
...
이전에 사용한 Fronted용 container image와 Backend용 container image 모두에 대해 실행합니다.
...
Frontend를 위한 container image를 build하고 push 했다면, Backend를 위한 container image도 동일한 과정을 진행합니다.
